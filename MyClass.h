...
// Mon May 23 21:29:12 2016 by ROOT version 6.06/00
// from TTree EventTree/Event data
// found on file: ggtree_data_429.root
//////////////////////////////////////////////////////////
...
class MyClass {
 public :
  ...
  MyClass(TString inputfile);
  virtual ~MyClass();
  virtual Int_t    Cut(Long64_t entry);
  virtual Int_t    GetEntry(Long64_t entry);
  virtual Long64_t LoadTree(Long64_t entry);
  virtual void     Init(TChain *tree);
  virtual void     Loop(string outputfile, int ntotEvent, int nPrintEvent);
  virtual Bool_t   Notify();
  virtual void     Show(Long64_t entry = -1);
  // Function Included by User
  float  DeltaPhi(float,float);
  float  dRCal(float, float, float, float);
  bool  MediumPhoton(int& );
  bool  MediumPhotonWtPixelSeed(int& );
  bool  VeryLoosePhoton(int& );
  bool  NonIsoLoosePhoton(int& );
  bool  PassedKinematicCut(int, float, float);
  //  bool  Collision(int);

  bool  HasRequiredPhoton(int& , float, float);
  bool  HasJetFakePhoton(int& , float, float);
  bool  HasElectronFakePhoton(int& , float, float);
  ...
  Double_t EcalEA(Double_t);
  Double_t EcalEA_ptscale(Double_t , Double_t );
  Double_t HcalEA(Double_t ) ;
  Double_t HcalEA_ptscale(Double_t , Double_t ) ;
  Double_t TkrEffAreas(Double_t);

  bool  electron_veto_looseID(int , float);
  //  bool  muon_veto_looseID(int , float);
  //  bool OverlapWithMuon(float, float);
  bool OverlapWithElectron(float, float);
  vector<int> JetVetoDecision(int);
  bool dPhiJetMET_veto(vector<int>);
...
#ifdef MyClass_cxx
MyClass::MyClass(TString inputfile)
{
  // if parameter tree is not specified (or zero), connect the file
  // used to generate this class and read the Tree.
  //if (tree == 0) {
  // TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("ggtree_data_429.root");
  // if (!f || !f->IsOpen()) {
  //   f = new TFile("ggtree_data_429.root");
  // }
  // TDirectory * dir = (TDirectory*)f->Get("ggtree_data_429.root:/ggNtuplizer");
  // dir->GetObject("EventTree",tree);

  //  TString inputfile = infile;

  /*
  cout<<" start: Input file : " << inputfile << endl;
  ...
  Init(tree);
  cout<<" End : Input file : " << inputfile << endl;
  */
  ...
  TSystemDirectory sourceDir("hi",inputfile);
  ...
  while ((filename = (TSystemFile*)next()) && fileNumber >  maxFiles)
    {
      ...
      TString  FullPathInputFile = (inputfile+filename->GetName());
  ...
  std::cout<<"No of Files: "<<fileNumber<<std::endl;
